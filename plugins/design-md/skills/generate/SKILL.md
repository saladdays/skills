...
**品質の内部分類（出力には露出しない）:**
- 生成した各ルールを内部で以下に分類する:
  - **L1（機械的に検証可能）:** 値の範囲チェック、コントラスト比、最小サイズ等。例: 「コントラスト比 4.5:1 以上」。L1 ルールは検証条件を具体値で明示する（例: 「≥ 4.5:1」「≤ 12px」）
  - **L2（目視で検証可能）:** スクリーンショットで確認できる。例: 「1ビューにフォーカルポイントは1つ」
  - **L3（主観的判断）:** 解釈者によって結論が分かれうる。例: 「堅すぎない信頼感」
- L3 が全ルールの50%を超える場合、具体的な判定基準または例示を補って L2 以下に引き下げる努力をする
- この分類ラベルは DESIGN.md の出力には含めない
