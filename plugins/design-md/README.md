...
必要に応じて、DESIGN.md から W3C Design Tokens Community Group 形式の `design-tokens.json` を出力して、Claude Design を含むトークン対応ツールに渡すこともできます。ただし、これはあくまで下支えの交換フォーマットです。DESIGN.md の本体は、トークンだけでは持てない「なぜその判断か」「迷ったときに何を優先するか」まで含めた判断レイヤーにあります。実際にやってみると、プロトからプロダクション実装まで判断の地続き感がそのまま残るのが気持ちいいです。
