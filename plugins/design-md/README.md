...
## Claude Design と組み合わせると最強

[Claude Design](https://support.claude.com/en/articles/14604397-set-up-your-design-system-in-claude-design) で会話しながら画面を作って、できあがったトークンや画面の判断材料を DESIGN.md に落とし込むと、その後の Cursor や Claude Code での実装でも同じ判断がそのまま引き継がれます。

逆に、DESIGN.md から必要に応じて W3C Design Tokens Community Group 形式の `design-tokens.json` を出力すれば、Claude Design を含むトークン対応ツールに渡せます。実際にやってみると、プロトからプロダクション実装まで判断の地続き感がそのまま残るのが気持ちいいです。
...
既存のサービスURLやFigmaのリンクを渡すと、今のデザインを読み取ったうえで DESIGN.md をつくってくれます。W3C DTCG 形式の JSON や CSS 変数のトークンファイルがあれば、それを下敷きにして整えることもできます。もちろん、まだ何もない状態からでも大丈夫です。
...
`/design-md:generate` をもう一度実行するだけです。既存の DESIGN.md を自動で読み取り、簡易診断を行います:

- **Creative North Star**: メタファーが今のプロダクトの方向性と合っているか
- **経過期間**: 3ヶ月以上経っていたら全体見直しを推奨
- **ルールの健全性**: 例外が増えすぎたルールには再定義を提案

診断結果をもとに、必要な箇所だけ更新できます。
...
| セクション | ひとことで言うと |
|---|---|
| **TL;DR** | デザインを5行でまとめたもの。絶対に妥協しない「死守ライン」を含めることも |
